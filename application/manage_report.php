...
class Manage_report extends CI_Controller {
	...
	function package_mode()
	{
		...
		{
			if($this->form_validate_mode() == FALSE)
					{
		...
		$data['maincontent']=$this->load->view('maincontents/add-edit-report-package-mode-view',$data,true);
		...
		if($this->input->post('slider1')==1)
		{
			if($this->form_validate_addon_duration() == FALSE)
					...
					else
					{
						...
						$q = "select pkg_payment.*,client.*,pkg_payment.id from pkg_payment inner join client on client.id=pkg_payment.c_id where pkg_payment.paydate>'$from_date' and pkg_payment.paydate<'$to_date'";
