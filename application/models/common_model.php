...
################################ FETCH DATA ########################################
	...
	function find_data($table,$return_type='array',$list=NULL,$conditions='',$select='*',$join='',$group_by='',$order_by='',$limit=0,$offset=0,$or_where='',$or_like='')
	{
...
############################## INSERT/UPDATE DATA ##################################
	...
	function save_data($table,$postdata = array(),$id=0)
	{
		if($id == 0)
		{
...
#####################################################################################	
	...
	// This Funcwtion To Get Last Employee Id 
	public function last_emp_id()
	{
	...
	// This Function update package assign table when renew a packge
	public function update_package($client_id,$object)
	{
		$this->db->where('c_id', $client_id);
		$this->db->update('pkg_assign', $object);
		
	}

	// This Function For Get Package Data By Package Name
	public function get_package_data($package_name)
	{
		$this->db->where('pkg', $package_name);
